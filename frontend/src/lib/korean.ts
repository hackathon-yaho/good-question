...
/**
 * 이름 뒤에 붙는 주격 조사 `이` / `가`를 고른다.
 *
 * 아이가 읽는 화면에 `이(가)`를 그대로 두면 어색하다 — "며느리이(가) 말하고 있어요"는
 * 사람이 쓰지 않는 문장이다. 캐릭터 이름은 이야기마다 바뀌므로 문구를 손으로 고를 수 없다.
 *
 *   방귀쟁이 며느리 → 며느리**가**    시아버지 → 시아버지**가**
 *   마을 이장 → 이장**이**            민준 → 민준**이**
 *
 * ⚠️ 조사가 필요한 곳마다 이걸 쓴다. `이(가)` 표기를 새로 만들지 않는다.
 */
...
/**
 * 이름 뒤에 붙는 주제 조사 `은` / `는`을 고른다.
 *
 * 미션 2의 문장 틀 `"{선택한 친구}는 ______ 할 수 있어요."`에 쓴다.
 * 지금 네 라벨은 모두 "친구"로 끝나 받침이 없지만, 라벨이 바뀌어도 문장이
 * 깨지지 않아야 한다. (화면 명세 C-11)
 */

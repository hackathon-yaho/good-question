...
| 백엔드 → AI `/analyze` | AI 서버 전체 10초 / 백엔드 연결 대기 10초 | AI 서버 최대 3회(최초 포함) / 백엔드 0회 |
| 백엔드 → AI `/respond` | AI 서버 전체 10초 / 백엔드 연결 대기 10초 | AI 서버 최대 3회(최초 포함) / 백엔드 0회 |
| 백엔드 → Whisper | **8초** | 0회 |
...
| `GUIDED` | `remainingWorry`를 대사에 녹여 부족한 요소를 유도한다. 저정보 반복이면 장면 질문 하나로 대화를 다시 잇는다 |

...
### 2.5 타임아웃 ✅ 재확정 (2026-08-15)

기존 D-03의 5초·재시도 0회는 [AI 호출 재시도·시간 예산 v2 요청](../request/backend/ai-retry-deadline-v2.md)으로
대체한다. API 스키마는 바뀌지 않는다.

| 구간 | 값 | 재시도 |
| --- | --- | --- |
| 프론트 → 백엔드 | 15초 (**실측 후 재검토**) | — |
| 백엔드 → AI `/analyze` | AI 서버 전체 10초 / 백엔드 연결 대기 11초 | AI 서버 최대 3회(최초 포함) / 백엔드 0회 |
| 백엔드 → AI `/respond` | AI 서버 전체 10초 / 백엔드 연결 대기 11초 | AI 서버 최대 3회(최초 포함) / 백엔드 0회 |
| 백엔드 → Whisper | **8초** | 0회 |
...
| 타임아웃 | ✅ AI 서버 요청 전체 10초 / 백엔드 연결 대기 11초 ([v2 요청](../request/backend/ai-retry-deadline-v2.md)) |
...
**응답 시간 예산** — STT는 별도 요청으로 분리되어 있지만, AI 재시도로 인한 최악 시간은
실측 전까지 보장하지 않는다.
...
콜드 스타트는 별도로 워밍업으로 줄이되, 이 정책을 근거로 15초 이내를 약속하지 않는다.
실측 전에는 클라이언트의 대화 요청 중단 시간을 새로 만들거나 줄이지 않는다.
...
| ~~백엔드→AI 타임아웃·재시도·실패 응답~~ | ✅ 전체 10초 / AI 서버 최대 3회 / 안전 폴백 — 2.5절 · v2 요청 |

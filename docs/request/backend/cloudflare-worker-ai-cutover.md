...
`/analyze`·`/respond`·`/report`·`/health` 계약, OpenAI 모델·재시도·실패 코드를 유지하며 DB·세션·미션·이미지를
소유하지 않는다.
...
[재시도·시간 예산 v2](ai-retry-deadline-v2.md)를 그대로 따른다. 보호자 리포트 계약은
...
| `POST {AI_SERVER_BASE_URL}/report` | `X-Internal-Token` | 저장된 규칙 기반 보호자 리포트 유지 |
| `GET {AI_SERVER_BASE_URL}/health` | 없음 | 배포 상태 확인만 사용 |

## 제약 조건

- 백엔드는 AI 서버를 재시도하지 않는다. AI Worker가 `/analyze`·`/respond`는 전체 10초, `/report`는 전체 60초 안에서 최초 포함 최대 3회만 시도한다.
...
- [ ] `/report`가 동일 토큰 헤더로 Worker의 200 응답을 받고, 실패 시 규칙 기반 리포트가 유지되는 것을 확인했다.

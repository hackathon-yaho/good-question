...
> **시간 제한·재시도 정책은 [AI 호출 재시도·시간 예산 v2](ai-retry-deadline-v2.md)로 대체됩니다.**
> 이 문서의 요청·응답 스키마, 인증, 상태 소유권은 그대로 유지합니다.
...
```json
{
  "status": "ok",
  "model": "gpt-5-mini",
  "promptVersions": { "analyze": "analyze_v3", "respond": "respond_v7" }
}
```
...
- 응답 시간·재시도: [v2 요청](ai-retry-deadline-v2.md)의 10초 전체 제한·최대 3회 정책을 따른다.

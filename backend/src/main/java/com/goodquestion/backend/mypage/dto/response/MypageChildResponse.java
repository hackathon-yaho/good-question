...
public record MypageChildResponse(UUID id, String name, String avatarId, int age) {

    public static MypageChildResponse of(Child child) {
        int age = Year.now().getValue() - child.getBirthYear();
        return new MypageChildResponse(child.getId(), child.getName(), child.getAvatarId(), age);
    }
}

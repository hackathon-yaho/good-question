...
| B-16 | ~~`GET /api/tts?messageId=` — 오디오 반환~~ | 필수 | **완료.** `voice/controller/TtsController.java`. 캐시 히트 시 즉시, 미스 시 생성 후 저장. `?text=`도 지원(D-26, `messageId` 없는 내레이션/단어 발음용). 둘 다 없으면 400 |

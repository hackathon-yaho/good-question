...
## Phase 8 · 보호자 리포트 AI 고도화 (신규, 설계 완료 · 구현 대기)

**선행**: Phase 7의 보호자 리포트(완료). 착수 조건은 AI팀이
[parent-report-ai-generation.md](../../docs/request/ai/parent-report-ai-generation.md) 확인·합의.

| 순서 | 작업 | 항목 |
| --- | --- | --- |
| 1 | AI팀 확인 대기 — 입출력 스키마·품질 기준·응답시간(60초 초기값) 합의 | [request/ai/parent-report-ai-generation.md](../../docs/request/ai/parent-report-ai-generation.md) |
| 2 | 백엔드: 세션 완료 시 규칙 기반 리포트 동기 생성(기존 유지) + AI 호출 백그라운드 트리거 + 성공 시 리포트 행 덮어쓰기 | 미착수 |
| 3 | AI 실패 시 규칙 기반 리포트가 그대로 유지되는지 통합 테스트 | 미착수 |
...
> 표현·근거 선정·질문 생성만" (분류·인용 오류를 구조적으로 차단). 상세 근거는 request 문서 자체에
> 있다 — decisions.md에는 별도 D-번호를 안 붙였다(아직 구현 전, AI팀 확인 대기 상태라 결정이 아니라
> 진행 중인 설계).

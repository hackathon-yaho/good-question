...
| 4 | ~~보호자 리포트 + `reports`~~ | O-01~O-05 — **완료** |
...
> **보호자 리포트 완료** (2026-08-12). 사용자가 준 리포트 가이드를 확인하는 과정에서
> `docs/reference/guardian-report-guide.md`(주최측 원문)와 프론트의
> `frontend/src/lib/api/{types.ts, mock-parent.ts}`가 이미 응답 형태·계산 로직·문구를 전부
> 갖추고 있음을 발견했다 — 추론 없이 그대로 포팅했다. api.md 3.8에 응답 예시 3개를 확정
> 반영하고, `reports` 테이블 저장 여부·선택 알고리즘 재사용 여부를 사용자에게 확인한 뒤
> `parent/{controller,service,dto,entity,repository,report}`를 구현했다. 세션을 대화4까지
> 완주해 리포트 생성부터 3개 GET 엔드포인트, 403/404 에러 경로까지 curl로 확인. 단위 테스트
> 18건(`ReportGeneratorTest`) 추가. 상세는 decisions.md D-24, work-items.md 12장.

---

## Phase 8 · 보호자 리포트 AI 고도화 (백엔드 구현 완료 · AI Worker 연동 대기)

**선행**: Phase 7의 보호자 리포트(완료).

| 순서 | 작업 | 항목 |
| --- | --- | --- |
| 1 | AI팀 확인 대기 — 입출력 스키마·품질 기준·응답시간(60초 초기값) 합의, 실제 `/report` 구현 | [request/ai/parent-report-ai-generation.md](../../docs/request/ai/parent-report-ai-generation.md) — **대기 중** |
| 2 | ~~백엔드: 세션 완료 시 규칙 기반 리포트 동기 생성(기존 유지) + AI 호출 백그라운드 트리거 + 성공 시 리포트 행 덮어쓰기~~ | D-55 — **완료** |
| 3 | ~~AI 실패 시 규칙 기반 리포트가 그대로 유지되는지 통합 테스트~~ | D-55 — **완료** |

> **배경** (2026-08-16). 배포 환경 실응답으로 `competencies[]` 5개 카드의 `evidence`가 세션
> 전체에서 가장 긴 발화 1개를 전부 동일하게 재사용하는 문제, `guide` 질문이 "방귀 뀌는 며느리"
> 이야기 하나에 하드코딩된 문제를 확인했다. `ReportGenerator`(규칙 기반, 카테고리당 문장 2택1)는
> 삭제하지 않고 AI 실패 시 안전망으로 남긴다 — API 응답 스키마는 안 바뀐다(`evidence`는 여전히
> 단일 문자열, `guide` 질문도 각 2개 고정). 설계는 그릴링(질문·결정 하나씩 확정)으로 진행했고,
> 핵심 원칙은 "사실 판정(카테고리 matched 여부)은 백엔드, 발화 원문도 백엔드가 채움 — AI는 문장
> 표현·근거 선정·질문 생성만" (분류·인용 오류를 구조적으로 차단).

> **백엔드 구현 완료** (2026-08-16, D-55). AI Worker의 실제 `/report`가 아직 없어 로컬
> mock(`AiMockController`)에 같은 스키마로 구현해 전체 파이프라인을 먼저 완성·검증했다.
> 이야기 1편을 처음부터 끝까지 완주해 `submitRetelling` 직후 리포트가 AI 버전으로 조용히
> 덮어써지는 것, 5개 역량 카드의 근거가 카테고리마다 달라지는 것(배포에서 봤던 "전부 동일 문장"
> 문제 해소), AI 실패 시 규칙 기반 버전이 그대로 남는 것까지 실호출로 확인. 구현 중 발견한
> 함정(트랜잭션 커밋 전에 `@Async`를 직접 부르면 백그라운드 스레드가 방금 저장한 리포트를
> 못 볼 수 있음)은 `@TransactionalEventListener(AFTER_COMMIT)`로 해결했다. 상세는 decisions.md
> D-55. **남은 건 AI팀의 실제 `/report` 엔드포인트뿐** — 그게 붙으면 `AI_SERVER_BASE_URL`만
> 실제 주소로 바뀌고 백엔드 코드는 안 바뀐다.
